...
async def test_app_client(async_client):
    """Test that the app client is initialized correctly."""
    response = await async_client.get("/v1/health", headers={"Content-Type": "application/json"})
...
async def test_dependencies_initialized(fastapi_app):
    """Test that all required dependencies are initialized."""
    # 验证应用状态中的关键依赖
    assert hasattr(fastapi_app.state, "async_session_maker"), "async_session_maker not initialized"
    assert hasattr(fastapi_app.state, "qdrant_client"), "qdrant_client not initialized"
    assert hasattr(fastapi_app.state, "redis_client"), "redis_client not initialized" 